...
def read_random_puzzle(driver,puzzle_size):

    #Open numberlink website
    ...
    puzzle_num = puzzle_lengths[puzzle_size]

    #Select puzzle size
    # size_menu = driver.find_element_by_xpath("//*[@id='container_left']/form/table/tbody/tr/td[3]/div/select") #open drop down menu
    # size_menu.click()
    # driver.implicitly_wait(10)
    option_xpath = "//*[@id='container_left']/form/table/tbody/tr/td[3]/div/select/option[%d]" %(int(puzzle_num)+1)
    ...
    #Start Puzzle
    for i in range(2):
        search = driver.find_element_by_class_name("button_green")
        search.click()
        driver.implicitly_wait(10)
...
def main():
    ...
    if len(sys.argv) != 3:
        sys.exit("Usage: python3 numberlink.py puzzle_size auto") #(1)
        sys.exit("Usage: python3 numberlink.py puzzle.csv file")  #(2)
    if sys.argv[2] == 'file':
        ...
        driver = Chrome()
        puzzle = read_random_puzzle(driver,puzzle_size)
